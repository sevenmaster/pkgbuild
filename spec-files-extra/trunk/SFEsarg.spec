...
# 
# 1,16,31,46 * * * *  sarg-reports today
# 10 1 * * 1,2,3,4,5 *          sarg-reports daily
# 10 1 * * 6         *          sarg-reports daily; sarg-reports weekly
# 20 1 * * *         1          sarg-reports  monthly
# 
# cp -p /source/usr/bin/sarg-reports /usr/bin/


##TODO## Paths in config files: modify to match the general install paths used
#        in this spec. e.g. /etc/squid/sarg.conf
...
perl -w -pi.bak -e "s,^#\!\s*/bin/sh,#\!/usr/bin/bash," configure sarg-reports
##TODO## add "export PATH=/usr/gnu/bin:$PATH" to get "gnu"-date and others found first
...
%changelog
* Sun Nov  4 2011 - Thomas Wagner
- bump to 2.3.2
- add apache config sample for sarg
