...
Name:           SFEnetatalk3
IPS_package_name:       service/network/netatalk3
...
%build
export CFLAGS="%optflags -xc99=all %{gnu_lib_path}"
./configure --prefix=%{_prefix}                  \
	    ...
	    --localstatedir=%{_localstatedir} \
            ...
            --with-ssl-dir=/usr \
            --enable-nfsv4acls \
	    --with-libevent-header=%{gnu_inc} \
	    --with-libevent-lib=%{gnu_lib}
...
#Install manifest
install -d -m 0755 ${RPM_BUILD_ROOT}/var/svc/manifest/network
sed	-e "s@:BINDIR:@/usr/bin@" \
	-e "s@:SBINDIR:@/usr/sbin@" \
	-e "s@:ETCDIR:@/etc@" \
	-e "s@:NETATALK_VERSION:@3.0.1@" \
	${RPM_BUILD_DIR}/netatalk-%{version}/distrib/initscripts/netatalk.xml.tmpl \
	> ${RPM_BUILD_ROOT}/var/svc/manifest/network/netatalk.xml 
...
%dir %{_localstatedir}/netatalk
%dir %{_localstatedir}/netatalk/CNID
%{_localstatedir}/netatalk/README
%{_localstatedir}/netatalk/CNID/README
...
%class(manifest) %attr(0444, root, sys) /var/svc/manifest/network/netatalk.xml

%defattr (-, root, sys)
%config(noreplace) %verify(not md5 mtime size) %{_sysconfdir}/afp.conf
...
%changelog
* Mon Nov 5 2012 - A Hettinger
- Adapt 2.2.3 spec for 3.0.1
